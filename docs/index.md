...
Hand `jtb2026` an Excel workbook, a delimited file or a data frame, tell
it which columns to read and what to look for, and it returns the
matching rows — optionally writing them to a CSV on the way out.
...
study-site comments, used throughout the documentation.
...
Other arguments: `ignore_case`, `whole_word`, `regex`, several keywords
at once with `match = "any" | "all"`, and `add_match_info` to record
which column matched.

### Semantic search

Nobody in the table wrote “stomach upset”. All four nausea rows come
back anyway, ranked:
...
The score blends TF-IDF cosine similarity over stemmed, synonym-expanded
tokens with a fuzzy character-3-gram similarity — no model, no network,
same answer every time. Supply `synonyms` to teach it your vocabulary,
or `embed_fun` to hand the scoring to a real embedding model.
...
See
[`ci/README.md`](https://github.com/yh202109/jtb2026/blob/main/ci/README.md)
for what each script does and which workflow runs it.

## License

MIT. See [LICENSE.md](https://yh202109.github.io/jtb2026/LICENSE.md).
